{{ config(
    materialized = 'table'
    , alias = 'bronze_crm_usuarios'
) }}

-- GERADO por server/dbtCodegen.ts — sistema "crm", camada Bronze, tabela usuarios.
-- A regeração sobrescreve este arquivo.
-- Origem: source('datacore_raw', 'usuarios')  (dataset via DBT_RAW_DATASET)
-- Saída : <DBT_SCHEMA_BRONZE>.bronze_crm_usuarios  (renome + LGPD Art. 46 + dedup CDC)
