{{ config(
    ...
    , alias = 'bronze_datacoredbt_usuarios'
    , unique_key = 'id'
    , incremental_strategy = 'merge'
) }}

-- GERADO por server/dbtCodegen.ts — sistema "DataCoreDBT", camada Bronze, tabela usuarios.
-- A regeração sobrescreve este arquivo.
-- Origem: source('datacore_raw', 'usuarios')  (dataset via DBT_RAW_DATASET)
-- Saída : <DBT_SCHEMA_BRONZE>.bronze_datacoredbt_usuarios  (renome + LGPD Art. 46 + dedup CDC)
