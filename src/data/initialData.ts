...
export const INITIAL_PIPELINES: Pipeline[] = [
  {
    id: 'pipe-ecommerce-bq',
    name: 'Pipeline Vendas & Transações E-Commerce',
    description: 'Ingestão de transações em tempo real do gateway de pagamentos com mascaramento de CPF e cartões de crédito para o Google BigQuery.',
    category: 'Vendas & Finanças',
    status: 'active',
    trigger: 'event',
    mode: 'streaming',
    cloudProviders: ['gcp', 'aws'],
    lastRunAt: 'Há 2 minutos',
    nextRunAt: 'Fluxo Contínuo (Streaming)',
    slaTarget: 99.9,
    actualSla: 99.98,
    recordsProcessedToday: 1458200,
    avgLatencyMs: 142,
    monthlyCostUsd: 412.50,
    owner: 'Carlos Silva (Engenheiro Sênior)',
    containsPII: true,
    ...
    nodes: [
      {
        id: 'node-src-pg',
        type: 'source',
        title: 'PostgreSQL Vendas OLTP',
        subtitle: 'vendas_production (CDC Stream)',
        provider: 'aws',
        iconName: 'Database',
        x: 60,
        y: 180,
        status: 'success',
        config: {
          connector: 'PostgreSQL RDS Connector',
          tableOrBucket: 'clientes, pedidos, pagamentos_cartao',
          format: 'JSON / CDC Debezium'
        },
        metrics: { recordsIn: 1458200, recordsOut: 1458200, durationMs: 45 }
      },
      {
        id: 'node-raw-pg',
        type: 'raw_data',
        title: 'Raw Data Landing Zone',
        subtitle: 's3://corp-lakehouse-raw/vendas/',
        provider: 'aws',
        ...
        status: 'success',
        config: {
          tableOrBucket: 'corp-lakehouse-raw/vendas/staging/',
          format: 'Snappy Parquet Raw (Metadados CDC)'
        },
        metrics: { recordsIn: 1458200, recordsOut: 1458200, durationMs: 28 }
      },
      {
        id: 'node-bronze-pg',
        type: 'bronze',
        title: 'Camada Bronze (Validação & LGPD)',
        subtitle: 'Delta Lake • Deduplicação & Cifragem',
        provider: 'generic',
        ...
        status: 'success',
        config: {
          query: 'VALIDATE SCHEMA & DEDUPLICATE (id_pedido, cpf)',
          maskingRules: [
            { field: 'cpf_titular', piiType: 'cpf', method: 'partial_redact' },
            { field: 'numero_cartao', piiType: 'credit_card', method: 'sha256_hash' },
            { field: 'email_comprador', piiType: 'email', method: 'tokenization' }
          ]
        },
        metrics: { recordsIn: 1458200, recordsOut: 1392040, durationMs: 38 }
      },
      {
        id: 'node-silver-bq',
        type: 'silver',
        title: 'Camada Silver (BigQuery DW)',
        subtitle: 'corp_datalake.faturamento_vendas',
        provider: 'gcp',
        iconName: 'Boxes',
        x: 870,
        y: 180,
        status: 'success',
        config: {
          connector: 'BigQuery Storage Write API',
          destinationTable: 'corp_datalake.faturamento_vendas',
  ...
  {
    id: 'pipe-crm-snowflake',
    name: 'Customer 360 Ingestão & Mascaramento PII',
    description: 'Sincronização diária do CRM Salesforce e ERP SAP para o Snowflake com anonimização de dados cadastrais e registro de base legal.',
    category: 'Clientes & CRM',
    status: 'active',
    trigger: 'cron',
    cronExpression: '0 3 * * * (Diário às 03:00 BRT)',
    mode: 'batch',
    cloudProviders: ['snowflake', 'aws'],
    lastRunAt: 'Hoje às 03:14 BRT',
    nextRunAt: 'Amanhã às 03:00 BRT',
    slaTarget: 99.5,
    actualSla: 99.85,
    recordsProcessedToday: 684200,
    avgLatencyMs: 420,
    monthlyCostUsd: 285.00,
    owner: 'Mariana Duarte (Data Engineer)',
    containsPII: true,
    legalBasis: 'Art. 7º, I - Consentimento Expresso',
    version: 'v1.8.2',
    nodes: [
      {
        id: 'n1',
        ...
        subtitle: 'Contact & Account Object',
        provider: 'generic',
        iconName: 'Cloud',
        x: 80,
        y: 160,
        status: 'success',
        config: { connector: 'REST API Salesforce v58', format: 'JSON' }
      },
      {
        id: 'n2',
        type: 'lgpd_mask',
        title: 'Ofuscador de Dados Pessoais',
        subtitle: 'Anonimização RG, Tel e Nome',
        provider: 'generic',
        iconName: 'ShieldCheck',
        x: 380,
        y: 160,
        ...
        config: {
          maskingRules: [
            { field: 'nome_completo', piiType: 'name', method: 'partial_redact' },
            { field: 'rg_titular', piiType: 'rg', method: 'sha256_hash' },
            { field: 'telefone_celular', piiType: 'phone', method: 'partial_redact' }
          ]
        }
      },
      {
        ...
        subtitle: 'PROD_DW.CORE_CRM.DIM_CLIENTE',
        provider: 'snowflake',
        iconName: 'Sparkles',
        x: 740,
        y: 160,
        status: 'success',
        config: { connector: 'Snowflake Snowpipe', writeMode: 'merge_upsert' }
      }
    ],
    edges: [
  ...
  {
    id: 'pipe-telemetria-iot',
    name: 'Telemetria IoT & Logs em Tempo Real',
    description: 'Processamento massivo de telemetria de sensores industriais e microserviços Kubernetes para Data Lake S3 com agregação por minuto.',
    category: 'Infraestrutura & IoT',
    status: 'active',
    trigger: 'event',
    mode: 'streaming',
    cloudProviders: ['aws'],
    lastRunAt: 'Há 1 minuto',
    nextRunAt: 'Fluxo Contínuo',
    slaTarget: 99.95,
    actualSla: 99.99,
    recordsProcessedToday: 8940100,
    avgLatencyMs: 68,
    monthlyCostUsd: 680.20,
    owner: 'Lucas Ferreira (Tech Lead)',
    containsPII: false,
    ...
    nodes: [
      {
        ...
        subtitle: 'topic: telemetry_sensors_v2',
        provider: 'aws',
        iconName: 'Radio',
        x: 80,
        y: 160,
        status: 'success',
        config: { connector: 'Kafka Confluent Cloud', format: 'Avro' }
      },
      {
        id: 'niot2',
        type: 'aggregate',
        title: 'Agregador de Janela (Tumbling 1m)',
        subtitle: 'AVG(temp), MAX(pressao)',
        provider: 'generic',
        iconName: 'Layers',
        x: 420,
        y: 160,
        status: 'success',
        config: { aggregation: { groupBy: 'sensor_id, window_1m', metric: 'avg, max, stddev' } }
      },
      {
        id: 'niot3',
        type: 'destination',
        title: 'AWS S3 Parquet Lake',
        subtitle: 's3://corp-lake-iot/telemetria/',
        provider: 'aws',
        iconName: 'FolderArchive',
        x: 780,
        y: 160,
        status: 'success',
        config: { connector: 'AWS S3 Connector', format: 'Snappy Parquet' }
      }
    ],
    edges: [
      { id: 'e1', source: 'niot1', target: 'niot2', animated: true },
      { id: 'e2', source: 'niot2', target: 'niot3', animated: true }
    ]
  },
  {
    id: 'pipe-rh-folha',
    name: 'Folha de Pagamento & Benefícios RH Seguro',
    description: 'Pipeline de alta sensibilidade para consolidação de salários e dados bancários de colaboradores com pseudonimização estrita.',
    category: 'RH & Pessoal',
    status: 'paused',
    trigger: 'cron',
    cronExpression: '0 20 28 * * (Dia 28 às 20:00 BRT)',
    mode: 'batch',
    cloudProviders: ['azure'],
    lastRunAt: '28 de Ago, 20:15 BRT',
    nextRunAt: '28 de Set, 20:00 BRT',
    slaTarget: 99.9,
    actualSla: 99.70,
    recordsProcessedToday: 0,
    avgLatencyMs: 310,
    monthlyCostUsd: 92.40,
    owner: 'Beatriz Lima (DPO & Eng)',
    containsPII: true,
    legalBasis: 'Art. 7º, II - Obrigação Legal / CLT',
    version: 'v1.1.0',
    nodes: [
      {
        id: 'nrh1',
        type: 'source',
        title: 'Senior Sistemas RH API',
        subtitle: 'endpoint /colaboradores/folha',
        provider: 'azure',
        iconName: 'Server',
        x: 80,
        y: 160,
        status: 'idle',
        config: { connector: 'REST OAuth2 TLS 1.3' }
      },
      {
        id: 'nrh2',
        type: 'lgpd_mask',
        title: 'Criptografia & Tokenização LGPD',
        subtitle: 'Salários & Contas Bancárias',
        provider: 'generic',
        iconName: 'Lock',
        x: 420,
        y: 160,
        ...
        config: {
          maskingRules: [
            { field: 'salario_base', piiType: 'salary', method: 'tokenization' },
            { field: 'dados_bancarios', piiType: 'credit_card', method: 'sha256_hash' }
          ]
        }
      },
      {
        ...
        subtitle: 'dw_corporate.rh.folha_analitica',
        provider: 'azure',
        iconName: 'HardDrive',
        x: 780,
        y: 160,
        status: 'idle',
        config: { connector: 'Azure Data Lake Storage Gen2' }
      }
    ],
    edges: [
      { id: 'e1', source: 'nrh1', target: 'nrh2' },
      { id: 'e2', source: 'nrh2', target: 'nrh3' }
    ]
  }
];
