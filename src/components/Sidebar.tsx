...
export type ActiveTab = 'studio' | 'studio-gold' | 'auto-pipeline' | 'pipelines' | 'execucoes' | 'chat-dados' | 'governance' | 'finops' | 'rbac' | 'administracao';
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const navItems: { id: ActiveTab; label: string; icon: React.ReactNode; badge?: number; badgeColor?: string }[] = [
    ...
    {
      id: 'chat-dados',
      label: 'Converse com os dados',
      icon: <MessagesSquare className="w-4 h-4 text-indigo-600 shrink-0" />
    },
    {
      id: 'governance',
    ...
    },
    {
      id: 'administracao',
      label: 'Administração',
      icon: <Settings className="w-4 h-4 text-indigo-600 shrink-0" />
    }
  ];
