...
- Modelo incremental: no INÍCIO do SQL busque a marca d'água com \`{% if is_incremental() %}{% set v_max_dat_carga = max_dat_carga() %}{% endif %}\` e filtre a origem com \`{% if is_incremental() and v_max_dat_carga is not none %} WHERE _dat_carga > TIMESTAMP('{{ v_max_dat_carga }}') {% endif %}\` — só registros posteriores à maior _dat_carga já gravada. Não use \`(SELECT max(...) FROM {{ this }})\`.
